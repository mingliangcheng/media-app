    ...
    <template v-if="isCertify">
      <uni-list>
        <uni-list-item
          class="item"
          title="姓名"
          :rightText="userInfo.realNameAuth.realName"
        ></uni-list-item>
        <uni-list-item
          class="item"
          title="身份证号码"
          :rightText="userInfo.realNameAuth.identity"
        ></uni-list-item>
      </uni-list>
    </template>
    <template v-else>
      <view class="uni-content">
        <template v-if="verifyFail">
          ...
          <button type="primary" @click="retry" v-if="verifyFailCode !== 10013">
            重新开始验证
          </button>
...
<script>
import checkIdCard from '@/uni/uni-id-pages/common/check-id-card.js'
import mixin from '@/uni/uni-id-pages/common/login-page.mixin.js'
  ...
  methods: {
    ...
    async getFrvAuthResult() {
      ...
      try {
        ...
        const { errCode, ...rest } = res

        if (this.verifyFailContent) {
          console.log(
            `[frv-debug] 客户端刷脸失败，由实人认证服务查询具体原因，原因：${this.verifyFailContent}`,
          )
        }
