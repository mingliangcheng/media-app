...
import passwordMod from '@/uni/uni-id-pages/common/password.js'
...
export default {
  ...
  computed: {
    isEmail() {
      const reg_email = /@/
      const isEmail = reg_email.test(this.formData.email)
      return isEmail
    },
    isPwd() {
      const reg_pwd = /^.{6,20}$/
      const isPwd = reg_pwd.test(this.formData.password)
      return isPwd
    },
    isCode() {
      const reg_code = /^\d{6}$/
      const isCode = reg_code.test(this.formData.code)
      return isCode
    },
  },
  onLoad(event) {
  ...
  methods: {
    ...
    submit() {
      this.$refs.form
        .validate()
        .then((res) => {
          const { email, password, captcha, code } = this.formData
          uniIdCo
            .resetPwdByEmail({
              email,
              code,
